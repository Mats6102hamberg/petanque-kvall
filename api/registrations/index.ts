  ...
  if (req.method === "POST") {
    try {
      ...
      if (user.status === "pending") {
        return res.status(403).json({ message: "Din användare väntar på godkännande" });
      }

      if (user.status === "inactive") {
        return res.status(403).json({ message: "Ditt konto är inaktiverat" });
      }
      ...
      if (existing) {
        return res.status(400).json({ message: "Du är redan anmäld" });
      }
